...
void mesh::LoadMesh(string obj_file, int test_scene)
{
	FILE	*scene;
	...
	while (!feof(scene))
	{
		...
		else if (!strcmp(token, "mtllib"))
		{
			...
			LoadMtl(string(obj_database) + string(mat_file), test_scene);
		}
...
void mesh::LoadMtl(string tex_file, int test_scene)
{
...
void mesh::Init(const char* obj_file, int test_scene)
{
	...
	LoadMesh(string(obj_file), test_scene);		// 讀入 .obj 檔 (可處理 Material)
}
